...
- **Graphing calculator** ✅ — Solver/Grapher tab strip on `/playground`; backend `POST /api/playground/graph` samples a function via SymPy `lambdify`; client renders with Recharts (multi-series, zero-axis reference lines, infinity-clipping). 5 quick-example chips (parabola, cubic, sine, exp, rational). Math keypad reused.
- **Spaced repetition (SRS)** ✅ — every wrong attempt auto-creates an SRS card; 5-level Anki-style intervals (1d → 3d → 7d → 14d → 30d → graduated). Backend `GET /api/srs/stats`, `GET /api/srs/due`; Dashboard `ReviewCard` shows "N cards due now" with a "Start review" CTA that launches a deck-only exam.
